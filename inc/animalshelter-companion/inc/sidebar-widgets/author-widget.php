...
class animalshelter_author_widget extends WP_Widget {
...
'animalshelter_author_widget', 


// Widget name will appear in UI
esc_html__( '[ Animalshelter ] Author Widget', 'animalshelter-companion' ),

// Widget description
array( 'description' => esc_html__( 'Add author content', 'animalshelter-companion' ), ) 
);
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'Author Name', 'animalshelter-companion' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'textarea' ) ); ?>"><?php esc_html_e( 'Short Descriptions:' ,'animalshelter-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Facebook Url' ) ); ?>"><?php esc_html_e( 'Facebook Url:' ,'animalshelter-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Twitter Url' ) ); ?>"><?php esc_html_e( 'Twitter Url:' ,'animalshelter-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Dribbble Url' ) ); ?>"><?php esc_html_e( 'Dribbble Url:' ,'animalshelter-companion'); ?></label> 
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Behance Url' ) ); ?>"><?php esc_html_e( 'Behance Url:' ,'animalshelter-companion'); ?></label> 
...
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>"><?php _e( 'Image', 'animalshelter-companion' ); ?>:</label>
	<div class="animalshelter-media-container">
		<div class="animalshelter-media-inner">
			...
			<span class="animalshelter-no-image" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-noimg" <?php echo wp_kses_post( $no_img_style ); ?>><?php _e( 'No image selected', 'animalshelter-companion' ); ?></span>
		</div>
	
	<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image' ) ); ?>" value="<?php echo esc_attr( $image ); ?>" class="animalshelter-media-url" />

	<input type="button" value="<?php echo _e( 'Remove', 'animalshelter-companion' ); ?>" class="button animalshelter-media-remove" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-remove" <?php echo wp_kses_post( $img_style ); ?> />

	<?php $button_text = ( $image != '' ) ? __( 'Change Image', 'animalshelter-companion' ) : __( 'Select Image', 'animalshelter-companion' ); ?>
	<input type="button" value="<?php echo esc_html( $button_text ); ?>" class="button animalshelter-media-upload" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-button" />
	<br class="clear">
	</div>
</p>
<style>
.animalshelter-media-container {
	width: 98%;
}

.animalshelter-media-inner {
...
.animalshelter-media-inner img {
	max-width: 100%;
	height: auto;
}

.animalshelter-media-url {
	display: none;
}

.animalshelter-media-remove {
	float: left;
	width: 48%;
}

.animalshelter-media-upload {
	float: right;
	width: 48%;
}
</style>
...
jQuery(function($){
    ...
    wpshed_image_upload( '.animalshelter-media-upload' );
    ...
    wpshed_image_remove( '.animalshelter-media-remove' );
...
} // Class animalshelter_author_widget ends here


// Register and load the widget
function animalshelter_author_load_widget() {
	register_widget( 'animalshelter_author_widget' );
}
add_action( 'widgets_init', 'animalshelter_author_load_widget' );