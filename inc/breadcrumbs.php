...
if ( ! function_exists( 'animalshelter_breadcrumbs' ) ) {
    function animalshelter_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'animalshelter' )
        );
        $args      = apply_filters( 'animalshelter_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html .= '<a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'animalshelter' ).' <span class="lnr lnr-arrow-right"></span></a>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<span class="s-text17 bread-search">'.esc_html__('Search results for:','animalshelter') . get_search_query() . '</span>';
        } elseif ( is_404() ) {
            $html .= '<span>' . esc_html__( 'Error 404', 'animalshelter' ) . '</span>';
        } elseif ( is_home() ) {
            $html .= '<span class="s-text17">' . get_the_title( get_option( 'page_for_posts' ) ) . '</span>';
        }
        $html .= '</div>';
        $html = apply_filters( 'animalshelter_breadcrumbs_filter', $html );
