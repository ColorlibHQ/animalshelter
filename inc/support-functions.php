...
// Post Category
function animalshelter_post_cats() {
...
// Post Tags
function animalshelter_post_tags() {
...
// animalshelter comment template callback
function animalshelter_comment_callback( $comment, $args, $depth ) {
                ...
                <div class="author-info justify-content-between">
                    <div class="thumb justify-content-between d-flex">
                        ...
                        <div class="author">
                            ...
                            <div class="">
                                <h5><?php printf( __( '<span class="comment-author-name">%s</span> ', 'animalshelter' ), get_comment_author_link() ); ?></h5>
                                <p class="date"><?php printf( __('%1$s at %2$s', 'animalshelter'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'animalshelter' ), '  ', '' ); ?> </p>
                                 <?php if ( $comment->comment_approved == '0' ) : ?>
                                 <em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'animalshelter' ); ?></em>
                                  <br>
                                <?php endif; ?>
                            </div>
                            <div class="reply-btn">
...
add_filter('comment_reply_link', 'animalshelter_replace_reply_link_class');
function animalshelter_replace_reply_link_class( $class ) {
    $class = str_replace("class='comment-reply-link", "class='btn-reply text-uppercase", $class);
    return $class;
}

// Move comment field to bottom
function animalshelter_move_comment_field_to_bottom( $fields ) {
...
add_filter( 'comment_form_fields', 'animalshelter_move_comment_field_to_bottom' );
